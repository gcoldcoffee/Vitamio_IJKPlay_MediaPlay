...
public class MiniThumbFile {
  ...
  private RandomAccessFile miniThumbDataFile() {
    if (mMiniThumbFile == null) {
      ...
      if (!directory.isDirectory()) {
        if (!directory.mkdirs())
          ALog.e("Unable to create .thumbnails directory %s", directory.toString());
      }
  ...
  protected synchronized long getMagic(long id) {
    ...
    if (r != null) {
      ...
      try {
        ...
        ALog.e("Got exception checking file magic: ", ex);
      } catch (RuntimeException ex) {
        ALog.e("Got exception when reading magic, id = %d, disk full or mount read-only? %s", id, ex.getClass().toString());
      } finally {
        try {
          if (lock != null)
            lock.release();
  ...
  protected synchronized void saveMiniThumbToFile(byte[] data, long id, long magic) throws IOException {
    ...
    try {
      ...
      ALog.e("couldn't save mini thumbnail data for %d; %s", id, ex.getMessage());
      throw ex;
    } catch (RuntimeException ex) {
      ALog.e("couldn't save mini thumbnail data for %d, disk full or mount read-only? %s", id, ex.getClass().toString());
    } finally {
      try {
        if (lock != null)
          lock.release();
  ...
  protected synchronized byte[] getMiniThumbFromFile(long id, byte[] data) {
    ...
    try {
      ...
      ALog.e("got exception when reading thumbnail id = %d, exception: %s", id, ex.getMessage());
    } catch (RuntimeException ex) {
      ALog.e("Got exception when reading thumbnail, id = %d, disk full or mount read-only? %s", id, ex.getClass().toString());
    } finally {
      try {
        if (lock != null)
          lock.release();
