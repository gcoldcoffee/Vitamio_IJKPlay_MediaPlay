...
public class Vitamio {
      ...
      if (libs != null) {
        ...
        for (String L : getRequiredLibs()) {
          if (Arrays.binarySearch(libs, L) < 0) {
            ALog.e("Native libs %s not exists!", L);
            return false;
          }
        }
        ...
        try {
          ...
          ALog.i("isNativeLibsInited, APP VERSION: %d, Vitamio Library version: %d", appVersion, libVersion);
          if (libVersion == appVersion)
            return true;
        } catch (IOException e) {
          ALog.e("isNativeLibsInited", e);
  ...
  private static boolean extractLibs(Context ctx, int rawID) {
    ...
    ALog.d("loadLibs start " + version);
    File lock = new File(getLibraryPath() + LIBS_LOCK);
    if (lock.exists())
      lock.delete();
    String libPath = copyCompressedLib(ctx, rawID, "libarm.so");
    ALog.d("copyCompressedLib time: " + (System.currentTimeMillis() - begin) / 1000.0);
    ...
    try {
      ...
      ALog.e("Error creating lock file", e);
    } finally {
      ALog.d("initializeNativeLibs: " + inited);
      ALog.d("loadLibs time: " + (System.currentTimeMillis() - begin) / 1000.0);
